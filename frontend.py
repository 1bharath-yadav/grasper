...
def make_multipart_files(uploaded_files, questions_text: str, use_questions_file: bool):
    ...
    if use_questions_file:
        ...
        files["questions.txt"] = (
            "questions.txt", questions_text.encode("utf-8"), "text/plain")
...
def display_metrics_and_visuals(parsed: Dict[str, Any]):
    ...
    for k, v in parsed.items():
        ...
        if isinstance(v, str) and len(v) > 50 and is_base64_string(v.split(",")[-1]):
            ...
            images.append((k, v))
        elif lname.endswith("_chart") or lname.endswith("_graph") or lname.endswith("_image") or (isinstance(v, str) and v.startswith("data:image/")):
            images.append((k, v))
        elif isinstance(v, (int, float, str)) and not (isinstance(v, str) and len(v) > 500):
            metrics[k] = v
    ...
    if tables:
        ...
        for name, df in tables.items():
            ...
            st.download_button(
                f"⬇️ Download `{name}.csv`", data=csv, file_name=f"{name}.csv", mime="text/csv")
    ...
    if images:
        ...
        for name, b64 in images:
            try:
                ...
                raw = b64.split(
                    ",")[-1] if "," in b64 and b64.startswith("data:") else b64
...
st.markdown("Upload data files and ask questions to get instant analyses, visualizations and downloadable results.")
...
with st.sidebar:
    ...
    api_endpoint = st.text_input(
        "API Endpoint", value=DEFAULT_API, help="URL of the Grasper API endpoint (POST /api)")
...
with left:
    ...
    SUPPORTED_EXTS = [
        "zip", "tar", "tgz", "tar.gz", "tar.bz2", "tar.xz",  # ARCHIVE_EXTS
        "csv", "tsv", "txt", "json", "jsonl", "parquet", "pq", "feather", "arrow", "xls", "xlsx", "xlsm", "html", "htmlx", "htm",  # DATA_FILE_EXTS
        "png", "jpg", "jpeg", "gif", "bmp", "tiff", "mp3", "wav", "mp4", "avi", "mov", "mkv", "webm", "pdf"  # MEDIA_EXTS
    ]
            ...
            try:
                ...
                if resp.status_code != 200:
                    ...
                    if result is not None:
                        ...
                        if isinstance(answers, dict) and "generated_code" in answers and answers["generated_code"]:
                            with st.expander("🐍 Generated Python Code", expanded=False):
                                ...
                                st.download_button("⬇️ Download generated code", data=generated_code_str.encode(
                                    "utf-8"), file_name="generated_code.py", mime="text/x-python")

                        # Primary answer body (string or dict)
                        body = answers.get("answer") if isinstance(
                        ...
                        if parsed_answer is None:
                            ...
                            if isinstance(parsed_answer, list):
                                try:
                                    ...
                                    st.download_button(
                                        "⬇️ Download CSV", data=csv, file_name="result.csv", mime="text/csv")
                                except Exception:
    ...
    if st.session_state.history:
        for idx, entry in enumerate(st.session_state.history[:3]):
            ...
            with st.expander(f"{idx+1}. Result at {t}", expanded=False):
                ...
                st.download_button(f"⬇️ Download JSON #{idx+1}", data=json.dumps(entry["result"], indent=2).encode(
                    "utf-8"), file_name=f"result_{idx+1}.json", mime="application/json")
    else:
